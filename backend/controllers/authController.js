...
export const login = async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn('⚠️ SMTP Error: Failed to send OTP email. Falling back to console log.', emailError.message);
      res.json({
        message: 'OTP logged to server console (SMTP credentials not configured)',
        userId: user._id,
        isDemoMode: true,
      });
    }
...
export const verifyOtp = async (req, res) => {
  try {
    ...
    // Ensure there is an active OTP request
    if (!user.otpHash || !user.otpExpiresAt) {
      return res.status(400).json({ message: 'No active login request. Please request a new OTP.' });
    }
